...
enum class Heading {
    N {
        ...
        override fun transform(adjustFactor: Int): (Position) -> Position = { p -> p.copy(y = p.y + (1 * adjustFactor)) }
    },
    E {
        override fun anticlockwise() = N
        override fun clockwise() = S
        override fun transform(adjustFactor: Int): (Position) -> Position = { p -> p.copy(x = p.x + (1 * adjustFactor)) }
    },
    S {
    ...
    abstract fun transform(adjustFactor: Int = 1): (Position) -> Position
}